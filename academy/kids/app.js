...
function fresh(name, pin) { return {name,pin,xp:0,current:0,completed:[],skills:{},badges:[],attempts:0}; }
function normalize(value) {
  value.completed = [...new Set(value.completed.filter(i => Number.isInteger(i) && i >= 0 && i < lessons.length))];
  value.current = Number.isInteger(value.current) ? Math.max(0, Math.min(value.current, lessons.length - 1)) : 0;
  value.skills = {};
  value.completed.forEach(i => { const key = lessons[i].s; value.skills[key] = Math.min(10, (value.skills[key] || 0) + 1); });
  value.xp = value.completed.length * 25;
  value.badges = [[1,'Digital Explorer'],[5,'Web Ranger'],[10,'AI & Cyber Scout'],[lessons.length,'Future Creator']].filter(([n])=>value.completed.length>=n).map(([,name])=>name);
  value.attempts = Number.isInteger(value.attempts) && value.attempts >= 0 ? value.attempts : 0;
  return value;
}
function save() {
...
function firstUnfinished() { const i = lessons.findIndex((_,i)=>!p.completed.includes(i)); return i < 0 ? lessons.length - 1 : i; }
function unlocked(i) { return p.completed.includes(i) || i <= firstUnfinished(); }
...
function render() {
  ...
  $('worlds').innerHTML = worlds.map(world=>{
    ...
    const done = indices.filter(i=>p.completed.includes(i)).length;
    const target = indices.find(i=>!p.completed.includes(i)) ?? indices[0];
  ...
  $('parentStats').textContent = `${p.completed.length}/${lessons.length} oppdrag · ${p.xp} XP. Fremgangen lagres separat for hvert profilnavn i denne nettleseren. Bruk samme nettadresse og nettleser neste gang.`;
  mission();
}
function jump(i) {
...
function mission() {
  const i=p.current, lesson=lessons[i], done=p.completed.includes(i);
...
function answer(j) {
  if (!p || p.completed.includes(p.current)) return;
  const lesson=lessons[p.current];
  p.attempts++;
  if (j===lesson.c) {
    p.completed.push(p.current); normalize(p); save(); render();
    if (p.completed.length===lessons.length) $('result').textContent += ' Du har fullført første Academy Kids-reise!';
  } else {
...
function next() { if(p && p.completed.includes(p.current) && p.current<lessons.length-1) jump(p.current+1); }
