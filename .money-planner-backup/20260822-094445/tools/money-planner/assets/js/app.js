(() => {
  ...
  function checkIncomePlausibility(){
    ['incomeMain','incomePartner'].forEach(n=>{
      const el=form.elements[n]; if(!el)return;
      let note=el.parentElement.querySelector('.plausibility');
      const v=E.n(el.value);
      if(v>=150000){
        if(!note){note=document.createElement('small');note.className='plausibility';el.parentElement.appendChild(note)}
        note.textContent='⚠ Dette ser ut som det kan være en årslønn. Feltet skal inneholde netto inntekt per måned.';
      }else if(note) note.remove();
    });
  }
  function bankGuide(){
    const el=q('#bankPriority'), out=q('#bankAdvice'); if(!el||!out)return;
    const map={
      mortgage:'Sammenlign effektiv rente, gebyrer og total lånekostnad. Ny verdivurdering og lavere belåningsgrad kan styrke forhandlingsposisjonen.',
      daily:'Se på kortgebyrer, kontogebyrer, nettbank og vilkår – ikke bare boliglånsrenten.',
      savings:'Sammenlign faktisk sparerente, uttaksbegrensninger og eventuelle krav til andre produkter.',
      service:'Vurder tilgjengelig rådgivning og oppfølging, men sammenlign fortsatt pris og vilkår med markedet.'
    };
    out.textContent=map[el.value]||map.mortgage;
  }

  function render(){
    state=data(); result=E.calculate(state); actions=E.actions(state,result); checkIncomePlausibility();
    q('#metricIncome').textContent=E.money(result.income); if(q('#householdIncomeTotal')) q('#householdIncomeTotal').textContent=E.money(result.income)+'/mnd'; if(q('#asideIncome')) q('#asideIncome').textContent=E.money(result.income);
  ...
  q('#bankPriority')?.addEventListener('change',bankGuide); bankGuide();
