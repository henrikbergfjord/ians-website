...
module.exports = async function(context,req) {
  try {
    ...
    context.res=json(200,{generatedAt:new Date().toISOString(),admin:principal.userDetails||null,subscription:{idSuffix:subscriptionId.slice(-6)},currency:mtd.currency||previous.currency||'NOK',monthToDate:mtd.cost,projectedMonthEnd:projected,previousMonth:previous.cost,knownMonthTotal,projectedKnownMonthTotal,knownAnnualRunRate:projectedKnownMonthTotal*12,externalConfiguredTotal,externalCosts,warnings:[previousProps.unavailable&&'Forrige måneds kostnad kunne ikke hentes.',dailyProps.unavailable&&'Daglige kostnader kunne ikke hentes.',serviceProps.unavailable&&'Kostnader per tjeneste kunne ikke hentes.',groupProps.unavailable&&'Kostnader per Azure-ressursgruppe kunne ikke hentes.'].filter(Boolean),budget:budget>0?budget:null,budgetUsedPercent:budget>0?(mtd.cost/budget)*100:null,daily,services,resourceGroups,note:'Azure is actual usage. Microsoft 365 Family is a verified fixed annual subscription converted to monthly cost. Other external costs are included only when configured.'});
  } catch(err) { context.log.error('azure-cost',err); context.res=json(502,{error:'Could not retrieve Azure cost data.',diagnostic:err.message}); }
};
