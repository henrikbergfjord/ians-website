...
async function sendNotification(data, context, kind) {
  const connectionString = process.env.COMMUNICATION_SERVICES_CONNECTION_STRING;
  const sender = process.env.ACADEMY_EMAIL_SENDER || process.env.IANS_EMAIL_SENDER;
  const recipient = process.env.ACADEMY_EMAIL_RECIPIENT || process.env.IANS_EMAIL_RECIPIENT;
  ...
  const isAccess = kind === 'access' || kind === 'yearwheel-access';
  const isYearwheel = kind === 'yearwheel-access';
  const heading = isYearwheel ? 'Ny tilgangsforespørsel – IANS Årshjul' : isAccess ? 'Ny tilgangsforespørsel – Henrik Academy' : 'Ny kontaktmelding – Henrik Academy';
  ...
  const subject = isYearwheel ? `Ny Årshjul-tilgang – ${data.name}` : `${isAccess ? 'Ny tilgangsforespørsel' : 'Ny Academy-melding'} – ${data.name}`;
...
module.exports = async function (context, req) {
  try {
    ...
    const isYearwheelRequest = subject === 'Tilgang til IANS Årshjul' || source === '/sameie-aarshjul-tilgang.html';
    if (isAcademyRequest || isAcademyContact || isYearwheelRequest) {
      try { await sendNotification(entity, context, isYearwheelRequest ? 'yearwheel-access' : isAcademyRequest ? 'access' : 'contact'); }
      catch (emailError) { context.log.error('IANS message was stored, but email notification failed:', emailError); }
    }

    context.res = json(201,{ok:true,message:isYearwheelRequest?'Tilgangsforespørselen til Årshjul er mottatt.':isAcademyRequest?'Tilgangsforespørselen er mottatt.':isAcademyContact?'Takk. Meldingen er sendt til Henrik Academy.':'Takk. Henvendelsen er mottatt av IANS.'});
  } catch (e) {
    context.log.error(e);
