module.exports = async function (context, req) {
  ...
  // Trust only the principal supplied by the Static Web Apps authentication proxy.
  let principal;
  try {
    principal = JSON.parse(Buffer.from(req.headers?.['x-ms-client-principal'] || '', 'base64').toString('utf8'));
  } catch {}
  if (!principal || !principal.userId || !Array.isArray(principal.userRoles)) {
    context.res = { status: 401, headers: cors, body: { error: 'Microsoft sign-in required.' } };
    return;
  }
  const allowedRoles = ['axionreader', 'axionuser', 'axioncustomer', 'axionadmin', 'axionowner'];
  if (!principal.userRoles.some(role => allowedRoles.includes(String(role).toLowerCase()))) {
    context.res = { status: 403, headers: cors, body: { error: 'Approved Axion access required.' } };
    return;
  }
